...
namespace Neuro
{
    ...
    public class NeuralNetwork
    {
        ...
        {
            return Model.FeedForward(inputs);
        }
        ...
        public void Fit(List<Array> inputs, List<Array> outputs, int epochs = 1, int verbose = 1, Track trackFlags = Track.TrainError | Track.TestAccuracy, bool shuffle = true)
        {
//            List<Data> trainingData = new List<Data>();
//            int batchSize = inputs[0].BatchSize;

//            TFTensor[] inp = new TFTensor[inputs.Count];
//            for (int i = 0; i < inputs.Count; ++i)
//            {
//#if VALIDATION_ENABLED
//                if (inputs[i].BatchSize != batchSize) throw new Exception($"TFTensor for input {i} has invalid batch size {inputs[i].BatchSize} expected {batchSize}!");
//#endif
//                inp[i] = inputs[i];
//            }

//            TFTensor[] outp = new TFTensor[outputs.Count];
//            for (int i = 0; i < outputs.Count; ++i)
//            {
//#if VALIDATION_ENABLED
//                if (outputs[i].BatchSize != batchSize) throw new Exception($"TFTensor for output {i} has invalid batch size {outputs[i].BatchSize} expected {batchSize}!");
//#endif
//                outp[i] = outputs[i];
//            }
...
//            Fit(trainingData, inputs[0].BatchSize, epochs, null, verbose, trackFlags, shuffle);
        ...
        public void Fit(Array inputs, Array outputs, int batchSize = -1, int epochs = 1, int verbose = 1, Track trackFlags = Track.TrainError | Track.TestAccuracy, bool shuffle = true)
        {
//            int numberOfTensors = inputs.GetLength(0); // we treat first input tensors list as a baseline
//#if VALIDATION_ENABLED
//            for (int i = 0; i < inputs.Count; ++i)
...
//                {
//#if VALIDATION_ENABLED
//                    if (inputs[i][n].BatchSize != 1) throw new Exception($"TFTensor at index {n} for input {i} has multiple batches in it, this is not supported!");
//#endif
//                    inp[i] = inputs[i][n];
//                }
...
//                {
//#if VALIDATION_ENABLED
//                    if (outputs[i][n].BatchSize != 1) throw new Exception($"TFTensor at index {n} for output {i} has multiple batches in it, this is not supported!");
//#endif
//                    outp[i] = outputs[i][n];
//                }
...
//            string outFilename = $"{FilePrefix}_training_data_{Optimizer.GetType().Name.ToLower()}_b{batchSize}{(Seed > 0 ? ("_seed" + Seed) : "")}_{TFTensor.CurrentOpMode}";
//            ChartGenerator chartGen = null;
//            if (trackFlags != Track.Nothing)
//                chartGen = new ChartGenerator($"{outFilename}", $"{Name}\nloss=[{string.Join(",", LossFuncs.Select(x => x.GetType().Name))}] optimizer={Optimizer} batch_size={batchSize}\nseed={(Seed > 0 ? Seed.ToString() : "None")} tensor_mode={TFTensor.CurrentOpMode}", "Epoch");
...
//                File.WriteAllLines($"{outFilename}_log.txt", LogLines);
        ...
        //    {
        //        losses[i] = new TFTensor(outputs[i].TFShape);
        ...
        private int Seed;
        private delegate int AccuracyFunc(TFTensor targetOutput, TFTensor output);
        private AccuracyFunc[] AccuracyFuncs;
        private List<string> LogLines = new List<string>();
        internal static TFGraph ActiveGraph;
        public Flow Model;
    }

    public class ParametersAndGradients
    {
        public TFTensor Parameters;
        public TFTensor Gradients;
    }
}
